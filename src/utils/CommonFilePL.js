...
const CommonFilePL = ({
  ...
  const handleExportToPDF = ({ columns, data, fileName = 'Report', filters = {}, logo }) => {
    ...
    //   doc.setFontSize(8);
    //   doc.setTextColor('#555555');
    //   doc.text(userText, pageWidth - 15, pageHeight - 10, { align: 'left' });
    // }
    ...
    doc.roundedRect(2, 30, 292, 12, 2, 2, 'F'); // full-width row
    ...
    doc.text('From Date', 190, 35);
    doc.text('To Date', 225, 35);
    ...
    doc.text(fromDate, 190, 40);
    doc.text(toDate, 225, 40);
    // doc.setFontSize(10);
    ...
    const columnStyles = {
      0: { cellWidth: 70 }, // partyname
      1: { cellWidth: 25 }, // Doc No
      2: { cellWidth: 25 }, // Doc Date
      3: { cellWidth: 25 }, // Due Date
      4: { cellWidth: 100 }, // Inv Amount
      5: { cellWidth: 25 }, // Outstanding
      6: { cellWidth: 25 } // Total Due
    };
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        doc.text(`Generated On: ${dayjs().format('DD-MM-YYYY hh:mm A')}`, pageWidth - 15, pageH - 10, { align: 'right' });
