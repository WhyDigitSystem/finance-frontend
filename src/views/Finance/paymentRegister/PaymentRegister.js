...
const APOutstanding = () => {
  ...
  const handleDownloadPdf = ({ logo, columns, data, fileName, userName, formData }) => {
    ...
    doc
      .setFillColor('#e7ebeb')
      .roundedRect(boxX, 15, boxW, 10, 4, 4, 'F')
      .setTextColor('#34449B')
      .setFontSize(12)
    ...
    const bandX = 5;
    const bandY = 33;
    // const bandWidth = pageW - 28;
    const bandWidth = pageW - 10;
    const bandHeight = 10;
    ...
    const numericFields = ['creditLimit', 'outstanding', 'unadjusted', 'totalDue', 'amount', 'balance', 'totaldue'];

    const body = data.map((row) =>
      columns.map((col) => {
        const key = col.accessorKey;
        const raw =
          key === 'combinedVendorInfo' ? [row.subledgerCode, row.subledgerName, row.currency].filter(Boolean).join(' - ') : row[key];
