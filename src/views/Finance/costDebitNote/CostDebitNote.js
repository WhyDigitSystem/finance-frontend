  ...
  const getAllCostDebitNoteById = async (row) => {
    ...
    try {
      ...
      if (result) {
        ...
        console.error('Error: No data found for this ID.');
      }
    } catch (error) {
      console.error('Error fetching data:', error);
    }
    ...
    setChargerCostInvoice((prev) => {
      return prev.map((row, idx) => {
        if (idx === index) {
          ...
          const lcAmt = qty * rate * exRate;
          const billAmt = lcAmt / exRate;
          const gst = (lcAmt * (updatedRow.gstPercent || 0)) / 100;
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(errors).length === 0 && CostInvoiceValid) {
      ...
      const saveFormData = {
        ...
        // totChargesBillCurrAmt: formData.totChargesBillCurrAmt,
        // gstInputLcAmt: formData.gstInputLcAmt,
        // approveBy: loginUserName,
              ...
              <div className="row d-flex ml">
                ...
                /> */}
                  <Autocomplete
                    ...
                    sx={{ width: '100%' }}
                    size="small"
                    value={formData.supplierName ? partyName.find((c) => c.partyName === formData.supplierName) || null : null}
                ...
                <div className="col-md-3 mb-3">
                  <Autocomplete
                    ...
                    size="small"
                    fullWidth
                    value={originBillVo.find((bill) => bill.docId === formData.originBill) || null}
                    onChange={(event, newValue) => {
                      if (newValue) {
                        setFormData((prev) => ({
                          ...prev,
                                          ...
                                          <td className="border px-2 py-2">
                                            <input
                                              ...
                                              onChange={(e) => {
                                                ...
                                                if (numericRegex.test(value)) {
                                                  setChargerCostInvoice((prev) =>
                                                    prev.map((r) => (r.id === row.id ? { ...r, lcAmt: parseFloat(value) || 0 } : r))
                                                  );
                                                  ...
                                                  setCostInvoiceErrors((prev) => {
                                                    ...
                                                    newErrors[index] = {
                                                      ...newErrors[index],
                                                      lcAmt: 'Only numeric characters are allowed'
                                                    };
                                                    return newErrors;
                                                  });
                                                }
                                    ...
                                    <tbody>
                                      {chargeDetails.map((detail, idx) => (
                                        <tr key={idx}>
                                          ...
                                          <td style={{ width: '150px', textAlign: 'center' }}>{detail.lcAmt}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
