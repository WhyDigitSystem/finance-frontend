  ...
  const getAllCostEstimateByOrgId = async () => {
    ...
    try {
      const result = await apiCalls(
        'get',
        `/costEstimation/getAllCostEstimationByOrgId?orgId=${orgId}&branchCode=${branchCode}&finYear=${finYear}`
      );
          ...
          for (let i = 0; i < detailsFromServer.length; i++) {
            ...
            if (localDetail?.supportingimg) {
              ...
              if (typeof localDetail.supportingimg === 'string') {
                ...
                file = base64StringToFile(localDetail.supportingimg, `image_${i}.jpg`);
              } else {
                // Handle new File objects
                file = localDetail.supportingimg;
              }

              await handleFileUpload(generatedId, serverDetail.id, file);
            }
          }
          handleClear();
          setLoading(false);
                ...
                <div className="col-md-3 mb-3">
                  <Autocomplete
                    ...
                    getOptionLabel={(option) =>
                      option?.employeeCode && option?.employeeName ? `${option.employeeCode} - ${option.employeeName}` : ''
                    }
                    disabled={formData.status === 'SUBMIT'}
                    value={empCodeList.find((item) => item.employeeCode === formData.empCode) || null}
                                    ...
                                    <tr key={row.id}>
                                      ...
                                      <td className="border px-2 py-2" style={{ width: '20%' }}>
                                        <Autocomplete
                                          ...
                                          value={categoryOptions.find((option) => option.category === row.category) || null}
                                          disabled={formData.status === 'SUBMIT'}
                                          onChange={(event, newValue) => {
                                      ...
                                      <td className="border px-2 py-2" style={{ width: '30%' }}>
                                        <Box display="flex" alignItems="center" gap={1}>
                                          ...
                                          >
                                            {row.supportingimg
                                              ? typeof row.supportingimg === 'object' && row.supportingimg.name
                                                ? row.supportingimg.name
                                                : 'Img👉'
                                              : 'Upload Img'}
                                          ...
                                          {row.supportingimg && (
                                            <IconButton
                                              variant="contained"
                                              sx={{ whiteSpace: 'nowrap', color: 'rgb(103 58 183)' }}
                                          ...
                                          >
                                            ...
                                            {row.supportingimg ? (
                                              <Box>
                                                <Avatar
                                                  src={
                                                    typeof row.supportingimg === 'object'
