...
import { TextField, Checkbox,Box,Typography,Button, FormControlLabel, FormHelperText, FormControl, InputLabel, MenuItem, Select, Radio, TableContainer } from '@mui/material';
...
<TableBody>
                {rowData.map((transaction, trxIndex) => {
                  // FIX 2: Handle both MIM and RIM data structures
                  const details = formData.type === 'MIM'
                    ? transaction.issueManifestProviderDetailsVOs
                    : transaction.retrievalManifestProviderDetailsVOs;
                          ...
                          <TableRow key={`${transaction.id}-${kitGroup.kitId}-${asset.id}`}>
                            ...
                            {kitIndex === 0 && assetIndex === 0 && (
                              <>
                                ...
                                {/* <TableCell rowSpan={totalAssetsInTransaction} align="right">
                                  {transaction.amount 
                                    ? `₹${Number(transaction.amount).toLocaleString('en-IN')}` 
                                    : '-'}
                                </TableCell> */}
