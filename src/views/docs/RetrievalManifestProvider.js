...
import { stringAndNoAndSpecialCharValidation, stringValidation } from '../../utils/userInputValidation';
...
function RetrievalManifestProvider({ addRim, rimId }) {
  ...
  const getRetrievalManifestProviderById = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const transformedKitDetails = editRimData.retrievalManifestProviderDetailsVOs.reduce((acc, detail) => {
          const existingKitIndex = acc.findIndex((kit) => kit.kitNo === detail.kitId);
  ...
  const getCustomersList = async () => {
    try {
      const response = await Axios.get(`${process.env.REACT_APP_API_URL}/api/master/getCustomersList?orgId=${orgId}`);

      if (response.status === 200) {
        setEmitterCustomersVO(response.data.paramObjectsMap.customersVO.emitterCustomersVO);
      }
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  ...
  const getWarehouseData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/warehouse/view?orgId=${orgId}`);
  ...
  const getAllKitData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/master/getallkit`);
  ...
  const handleEmitterChange = (event) => {
    ...
    const selectedReceiver = emitterCustomersVO.find((customer) => customer.id === parseInt(selectedReceiverId));
    ...
    if (selectedReceiver) {
      ...
      if (address) {
        ...
        setReceiverAddress(`${street1}, ${street2}, ${city}, ${state} - ${pinCode}`);
      ...
      <div style={{ maxWidth: 1060 }} className="ml-auto me-auto">
        <div className="card w-full p-6 bg-base-100 shadow-xl" style={{ padding: '20px' }}>
          ...
          <div className="d-flex justify-content-end">
            <IoMdClose onClick={handleMimClose} className="cursor-pointer mb-3" style={{ width: '32px', height: '32px' }} />
          </div>
          <div className="row">
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              <label className="label">
                <span className={'d-flex flex-row label-text label-font-size text-base-content'}>
                  Transaction No:
            ...
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              ...
              />
              {errors.transactionNo && <span className="error-text">{errors.transactionNo}</span>}
            </div>
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              <label className="label">
            ...
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              ...
              {errors.transactionDate && <span className="error-text">{errors.transactionDate}</span>}
            </div>
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              <label className="label">
                <span className={'label-text label-font-size text-base-content d-flex flex-row'}>
            ...
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              ...
              {errors.dispatchDate && <span className="error-text">{errors.dispatchDate}</span>}
            </div>
            <div className="col-lg-3 col-md-6 mb-2 col-sm-4">
              <label className="label">
                <span className={'label-text label-font-size text-base-content d-flex flex-row'}>
            ...
            <div className="col-lg-3 col-md-6 mb-2">
              <label className="label">
                <span className={'label-text text-base-content '}>Sender's Address:</span>
              </label>
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
              <textarea
        ...
        <div className="col-lg-3 col-md-6">
          ...
          {errors.sender && (
            ...
            <div className="col-lg-3 col-md-6 mb-2">
              <label className="label">
                <span className={'label-text text-base-content'}>Receiver Warehouse:</span>
              </label>
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
            ...
            <div className="col-lg-3 col-md-6 mb-2">
              <label className="label">
                <span className={'label-text label-font-size text-base-content '}>Transporter Name:</span>
              </label>
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
              ...
              {errors.transporterName && <span className="error-text">{errors.transporterName}</span>}
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
              <label className="label">
                <span className={'label-text label-font-size text-base-content '}>Vehicle No:</span>
              </label>
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
              <input
            ...
            <div className="col-lg-3 col-md-6 mb-2">
              <label className="label">
                <span className={'label-text label-font-size text-base-content '}>Driver No:</span>
              </label>
            </div>
            <div className="col-lg-3 col-md-6 mb-2">
              <input
                ...
                maxLength={10}
              />
              {errors.driverNo && <span className="error-text">{errors.driverNo}</span>}
            </div>
            <div className="col-lg-12 col-md-12 mb-2">
              <div className="d-flex justify-content-end">
                <button className="btn btn-ghost btn-lg text-sm col-xs-1" style={{ color: 'blue' }} onClick={() => setOpenKitModal(true)}>
                  <img
                    src="/new.png"
                    alt="new-icon"
                    title="new"
            ...
            <div className="d-flex justify-content-end">
              {errors.kitDetails && <span className="error-text mb-1">{errors.kitDetails}</span>}
            </div>
            {kitDetails.length > 0 && (
              <div className="w-full p-3 bg-base-100 shadow-xl" style={{ borderRadius: 16 }}>
                      ...
                      {kitDetails.map((kit, index) => (
                        ...
                        >
                          {index + 1}
                            ...
                            <td className="text-center" rowSpan={kit.assets.length + 1}>
                              <FaTrash
          ...
          <Dialog fullWidth={true} maxWidth={'md'} open={openKitModal} onClose={handleKitClose}>
            <div>
              <ToastContainer />
            </div>
            <div className="d-flex justify-content-between">
              <DialogTitle>Add Kit & Qty Details</DialogTitle>
              <IoMdClose onClick={handleKitClose} className="cursor-pointer mt-3 me-3" style={{ width: '32px', height: '32px' }} />
            </div>
            <DialogContent>
              <DialogContentText>
                <div className="row">
                  ...
                  <div className="col-lg-3 col-md-6 mb-2">
                    <label className="label">
                      <span className={'label-text label-font-size text-base-content '}>Kit Qty:</span>
                    </label>
                  </div>
                  <div className="col-lg-3 col-md-6 mb-2">
                    <input
                    ...
                    />
                    {errors.kitQty && <div className="error-text">{errors.kitQty}</div>}
                  </div>
                  <div className="col-lg-3 col-md-6 mb-2">
                    <label className="label">
                      <span className={'label-text label-font-size text-base-content '}>HSN Code:</span>
                    </label>
                  </div>
                  <div className="col-lg-3 col-md-6 mb-2">
                    <input
                    ...
                    />
                    {errors.hsnCode && <div className="error-text">{errors.hsnCode}</div>}
                  </div>
                </div>
              </DialogContentText>
            </DialogContent>
            <DialogActions className="mb-2 me-2">
              ...
              <Button component="label" variant="contained" onClick={handleAddKitDetails}>
                Submit
              </Button>
            </DialogActions>
          </Dialog>
